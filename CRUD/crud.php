...
/*******************************************************************************
    ...
    2.4.2 bw -- 2014-03-27 - Public release with support for mysql stored functions
    ...
    switch(DBENGINE) {
        ...
        case 'mysql':
            ...
            try {
                ...
                if($dbh) set_mysql_album_len_function($dbh);
            } catch (PDOException $e) {
                error($e->getMessage());
            }
            break;
...
function get_album_length($id)
{
    ...
    if(! in_array(DBENGINE, array('mysql', 'sqlite3', 'pgsql'))) return NULL;
    $dbh = $CRUD['dbh'];
    $query = '';
    switch(DBENGINE) {
        case 'mysql':
            $query = "SELECT album_len(sum(duration)) FROM track WHERE album_id = ?";
            break;
        case 'sqlite3':
